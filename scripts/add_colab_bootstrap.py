...
Colab loads a lab as a single file without the repo, so relative paths and any
course package are missing. This adds the canonical bootstrap (labs/_colab.py) as
the first code cell of every committed lab notebook, right after its title cell.
Re-running is safe: notebooks that already carry the @colab-bootstrap marker are
skipped.

Usage (from repo root):
    .venv/bin/python scripts/add_colab_bootstrap.py            # all labs/NNNN-*.ipynb
    .venv/bin/python scripts/add_colab_bootstrap.py labs/0008-pca-returns-panel.ipynb
"""
